...
An end-to-end data engineering pipeline that transform raw CSV files from Google Cloud Storage into a normalized star schema data warehouse, then builds analytical data marts.
...
- ✅ **Data modeling:** Design a **star schema** with fact tables, dimensions, and bridge tables for many-to-many relationships
- ✅ **ETL development:** Implemented **extract, transform, load** processes with idempotent operations and data quality checks
- ✅ **Mart architecture:** Create **specialized data marts** (flat, skills, priority) with additive measures and incremental update patterns
...
├──03_create_flat_mart.sql      # Denormalize flat mart
├──04_create_skills_mart.sql    # Skills demand mart
├──05_create_priority_mart.sql  # Priority roles mart
├──06_update_priority_mart.sql  # Priority_mart incremental update (MERGE)
...
- **SQL Files**:
  - [01_create_tables_dw.sql](./01_create_tables_dw.sql) - Define star schema with 4 core tables
...
Denormalize table with all dimensions for ad-hoc queries.

![Flat Mart](../Images/1_2_Flat_Mart.png)
- **SQL Files**: [03_create_flat_mart.sql](./03_create_flat_mart.sql) - Builds denormalized table with all dimensions joined
...
- **Key Feature**: All measures are additive (counts/sums) for safe re-aggregation
...
- **Key Feature**: **MERGE operations for incremental updates** - demonstrates production-ready upsert patterns (INSERT, UPDATE, DELETE in single statement)
...
- **Logic Handling:** `COALESCE` & `Cast Operation` logic conversion for aggregating flags (remote, health insurance, no degree)
