...
INSERT INTO skills_mart.dim_date_month (
    ...
    quarter, quarter_name, year_quarter
)
SELECT DISTINCT
...
SELECT '=== Loading Skill Demand Monthly Fact for Skills Mart ===' AS info;
...
INSERT INTO skills_mart.fact_skill_demand_monthly (
    ...
    postings_count, remote_postings_count,   
...
WITH job_postings_prep AS (
    SELECT
        ...
        COALESCE(jpf.job_work_from_home::INTEGER, 0) AS is_remote,
        COALESCE(jpf.job_health_insurance::INTEGER, 0) AS has_health_insurance,
        COALESCE(jpf.job_no_degree_mention::INTEGER, 0) AS no_degree_mentioned
...
UNION ALL SELECT 'Date Month Dimension', COUNT(*) FROM skills_mart.dim_date_month
UNION ALL SELECT 'Skill Demand Monthly Fact', COUNT(*) FROM skills_mart.fact_skill_demand_monthly;
...
SELECT '=== Skill Demand Monthly Fact Sample ===' AS info;
